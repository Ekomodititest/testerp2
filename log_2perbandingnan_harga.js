...
function getNoPQ(nopp){
		notransaksi = document.getElementById('notransaksi').value;
        param='notransaksi='+notransaksi+'&nopp='+nopp+'&proses=getnopq';
        tujuan='log_slave_2perbandingan_harga.php';
		  ...
		  if(con.readyState==4){
					if (con.status == 200) {
						...
						if (!isSaveResponse(con.responseText)) {
							...
							document.getElementById('notransaksi').innerHTML=trim(con.responseText);
...
function getNoPP(notransaksi){
		nopp = document.getElementById('nopp').value;
        param='notransaksi='+notransaksi+'&nopp='+nopp+'&proses=getnopp';
        tujuan='log_slave_2perbandingan_harga.php';
        post_response_text(tujuan, param, respog);
        function respog(){
		  if(con.readyState==4){
					if (con.status == 200) {
						...
						if (!isSaveResponse(con.responseText)) {
							...
							document.getElementById('nopp').innerHTML=trim(con.responseText);
...
function getReportPQ(){
		nopp = document.getElementById('nopp').value;
		notransaksi = document.getElementById('notransaksi').value;
        param='nopp='+nopp+'&notransaksi='+notransaksi+'&proses=preview';
        tujuan='log_slave_2perbandingan_harga.php';
        post_response_text(tujuan, param, respog);
        function respog(){
		  if(con.readyState==4){
					if (con.status == 200) {
						...
						if (!isSaveResponse(con.responseText)) {
							...
							document.getElementById('printContainer').innerHTML=trim(con.responseText);	
						}
					}
					else {
						busy_off();
