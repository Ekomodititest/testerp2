...
function findBarang(){
	...
	if(txt==''){
		alert(document.getElementById('field').value + " " + document.getElementById('alertrequired').value);
	}
	else{
		param='txtfind='+txt;
...
function simpanKonversi(){
	...
	if (dari == '' || ke == '' || parseFloat(jumlah) < 0.0001) {
		alert(parseFloat(jumlah) < 0.0001 + " " + document.getElementById('inconsistent').value);
	}
	else {
