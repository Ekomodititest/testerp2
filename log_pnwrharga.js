    ...
    for(i=1;i<=row;i++)
        {
            ...
            if(ar.checked==true)
            {
            minpq = document.getElementById('minpq_'+i).value;
                    try{
                            if(strUrl != '')
                            {
...
function addDataSma(){
	var cek = document.getElementById('id_supplier').value;	
	if(cek !="") {
...
function datakeExcel(ev,nmr,nourut)
{
        param='method=printExcel'+'&no_permintaan='+nmr+'&nourut='+nourut;
        //alert(param);
        tujuan='log_slave_save_permintaan_harga.php';
...
function zPreview2Edit2(fileTarget,notrans,idCont) {
	var r = confirm("Barang dalam PQ ini sudah ada di PO. Apakah anda yakin akan mengubah ?");
	if (r == false) {
		return;
	} else {
		if(notrans==''){
...
function zPreview2Edit(fileTarget,notrans,idCont) {
    if(notrans==''){
        notrans=document.getElementById('notransaksi').value;
    }
    param='method=preview2'+'&notransaksi='+notrans;   
...
{
    jmlh_brg=document.getElementById('count_'+id).innerHTML;
...
function calculate_dpp_ok(brs)
{
	DPPInpt = document.getElementById('dppokinpt_'+brs).value;	
	document.getElementById('dppok_'+brs).value = DPPInpt;
	document.getElementById('dppok_'+brs).disabled=true;
	
	TotalOngkosKirim(brs, DPPInpt);
	grandTotal(brs);
}

function calculateok(brs)
{
	PPNOkOpt = document.getElementById('ppn_ok_opt_'+brs).value;	
	PPHOkOpt = document.getElementById('pph_ok_opt_'+brs).value;
	TotalOk = document.getElementById('total_ok_'+brs).value;
	Pembagi = 1 + parseFloat(parseFloat(PPNOkOpt) - parseFloat(PPHOkOpt)) / 100;
	DPP = parseFloat(TotalOk) / Pembagi;

	document.getElementById('dppok_'+brs).value=Math.round(DPP);
	
	Hasilppnokopt = (parseFloat((DPP))*PPNOkOpt)/100;
	document.getElementById('ppn_ok_'+brs).value=Math.round(Hasilppnokopt);

	Hasilpphokopt=(parseFloat((DPP))*PPHOkOpt)/100;
	document.getElementById('pph_ok_'+brs).value=Math.round(Hasilpphokopt);
	
	grandTotal(brs);	
}

function TotalOngkosKirim(brs, DPP){
	PPNOk = document.getElementById('ppn_ok_'+brs).value;
	PPHOk = document.getElementById('pph_ok_'+brs).value;
	TotalOk = (parseFloat(DPP) + parseFloat(PPNOk) - parseFloat(PPHOk));
	document.getElementById('total_ok_'+brs).value=TotalOk;
}
...
function calculatePpn(brs)
{
        ...
        ppn=(parseFloat((subTot.innerHTML-dis.value))*ppN)/100;
        document.getElementById('ppn_'+brs).value=ppn;
		
		grandTotal(brs);
}

function calculateppnok(brs)
{
	calculateok(brs);
}

function calculatepbbkb(brs)
{
        var reg = /^[0-9]{1,2}$/;
		subTot=document.getElementById('total_harga_po_'+brs);
		dis=document.getElementById('angDiskon_'+brs);
        pbbkb_opt=document.getElementById('pbbkb_opt_'+brs).value;
        pbbkb=(parseFloat((subTot.innerHTML-dis.value))*pbbkb_opt)/100;
        document.getElementById('pbbkb_'+brs).value=pbbkb;
...
function calculatepph(brs)
{
	...
	subTot=document.getElementById('total_harga_po_'+brs);
	dis=document.getElementById('angDiskon_'+brs);
	pph_opt=document.getElementById('pph_opt_'+brs).value;
	pph=(parseFloat((subTot.innerHTML-dis.value))*pph_opt)/100;
	document.getElementById('pph_'+brs).value=pph;
	
	grandTotal(brs);
}

function calculatepphok(brs)
{
	calculateok(brs);
}
...
{
	sb_tot = parseFloat(document.getElementById('total_harga_po_'+brs).innerHTML);
	angDiskon = parseFloat(document.getElementById('angDiskon_'+brs).value);
	ppn = parseFloat(document.getElementById('ppn_'+brs).value);
	pbbkb = parseFloat(document.getElementById('pbbkb_'+brs).value);
	pph = parseFloat(document.getElementById('pph_'+brs).value);
	DPPOk = parseFloat(document.getElementById('total_ok_'+brs).value);
	//ppn_ok = document.getElementById('ppn_ok_'+brs).innerHTML;
	//alert(parseFloat(sb_tot)); 
	//alert(parseFloat(angDiskon)); 
	//alert(parseFloat(ppn)); 
	//alert(parseFloat(pbbkb)); 
	//alert(parseFloat(pph)); 
	//return;
	
	grnd_tot = parseFloat((sb_tot - angDiskon)) + parseFloat(ppn) + parseFloat(pbbkb) + parseFloat(pph) + parseFloat(DPPOk);
	total=document.getElementById('grand_total_'+brs);
    total.innerHTML=toRp(grnd_tot);
	
	totalkonversi=grnd_tot*(document.getElementById('Kurs_'+brs).value);
	totalkonv=document.getElementById('totalkonversi_'+brs);
	totalkonv.innerHTML=toRp(totalkonversi);	
}
...
function simpanSemua2(brs,totRow)
{
	...
	Garansi=document.getElementById('Garansi_'+brs).value;
	Penalty=document.getElementById('Penalty_'+brs).value;
	Lainlain=document.getElementById('Lainlain_'+brs).value;
	
    nilPPn=document.getElementById('ppn_'+brs).value;
    ppn_opt=document.getElementById('ppN_'+brs).value;
	...
	dppok	= document.getElementById('dppok_'+brs).value;
	totalOk	= document.getElementById('total_ok_'+brs).value;
	
	ppn_ok_opt	= document.getElementById('ppn_ok_opt_'+brs).value;
    ppn_ok		= document.getElementById('ppn_ok_'+brs).value;
	
	pph_ok_opt	= document.getElementById('pph_ok_opt_'+brs).value;
    pph_ok		= document.getElementById('pph_ok_'+brs).value;
	
	//alert(dppok);return;
    grand_total =document.getElementById('grand_total_'+brs).innerHTML;
	var nilaiPermintaan = grand_total.substring(0, grand_total.length - 2);
	nilaiPermintaan = nilaiPermintaan.replaceAll('.', '');
	
    subTotal=document.getElementById('total_harga_po_'+brs).innerHTML;
    termPay=document.getElementById('term_pay_'+brs).value;
    idFranco=document.getElementById('tmpt_krm_'+brs).options[document.getElementById('tmpt_krm_'+brs).selectedIndex].value;
    stockId=document.getElementById('stockId_'+brs).value;
    ...
    tgl_mask_pnwrn=document.getElementById('tgl_mask_pnwrn_'+brs).value;
    masa_berlaku=document.getElementById('masa_berlaku_'+brs).value;
	tglkrm=document.getElementById('tglkrm_'+brs).value;
    if((subTotal=='0')||(subTotal==''))
        {
            subTotal=nilDiskon=diskonPersen=nilPPn=0;
        }
    ...
    for(i=1;i<row;i++)
    {
                    try{
                            if(strUrl != '')
                            {
                                    ...
                                    +'&jmlh[]='+document.getElementById('count_'+i).innerHTML;
                            }
                            else
                            {
                                    strUrl += '&kdbrg[]='+encodeURIComponent(trim(document.getElementById('kd_brg_'+i).innerHTML))
    ...
    param+='&nilDiskon='+nilDiskon+'&diskonPersen='+diskonPersen+'&nilPPn='+nilPPn+'&ppn_opt='+ppn_opt+'&nilaiPermintaan='+nilaiPermintaan;
    param+='&subTotal='+subTotal+'&termPay='+termPay+'&idFranco='+idFranco+'&stockId='+stockId+'&ketUraian='+ketUraian;
    param+='&tgl_mask_pnwrn='+tgl_mask_pnwrn+'&masa_berlaku='+masa_berlaku+'&mtUang='+mtng+'&kurs='+krs+'&supplierId='+supplierId;
	param+='&tglkrm='+tglkrm+'&pbbkb_opt='+pbbkb_opt+'&pbbkb='+pbbkb+'&pph_opt='+pph_opt+'&pph='+pph;
	param+='&dppok='+dppok+'&totalOk='+totalOk+'&ppn_ok_opt='+ppn_ok_opt+'&ppn_ok='+ppn_ok+'&pph_ok_opt='+pph_ok_opt+'&pph_ok='+pph_ok;
	param+='&Garansi='+Garansi+'&Penalty='+Penalty+'&Lainlain='+Lainlain;
    param+=strUrl;
    tujuan='log_slave_save_permintaan_harga.php';
	//alert(param);return;
//  return;
...
function savepemenang(){
	pemenangar = document.getElementById('pemenang').options[document.getElementById('pemenang').selectedIndex].value;
	no_prmntan=document.getElementById('no_prmntan_1').value;
	
	param='method=savepemenang'+'&pemenangar='+pemenangar+'&no_prmntan='+no_prmntan;
	tujuan='log_slave_save_permintaan_harga.php';
	post_response_text(tujuan, param, respog);	
	function respog(){
            if (con.readyState == 4) {
                    if (con.status == 200) {
							...
							alert('Pemenang berhasil di pilih');
							reload.location();
                        }
                    }
                    else {
...
function diskurs(id){
	kurs = document.getElementById('mtUang_'+id).value;  
	defa = document.getElementById('check_'+id).value;  
	defb = document.getElementById('check1_'+id).value;  
	if(kurs=='IDR') {
		document.getElementById('Kurs_'+id).disabled = true;
		document.getElementById('Kurs_'+id).value = 1;
	} 
	else {
		document.getElementById('Kurs_'+id).disabled = false;
		if(kurs == defb){
			document.getElementById('Kurs_'+id).value = defa;
		} else {
			document.getElementById('Kurs_'+id).value = 0;
		}
	}
}