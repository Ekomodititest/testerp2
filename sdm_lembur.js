...
function addDetail() {
        if(status_inputan==0)
        {
                if(confirm('Apakah Anda yakin akan menyimpan Periode ini?'))
                {
                        cek_data();	
                }
        }
...
//	alert('test');
        if(confirm("Apakah Anda yakin akan mengedit data ini?"))
        {
...
function deleteDetail(id) {
        ...
        var rkrywn = document.getElementById('krywnId_'+id);
                ...
                function respon(){
                        ...
                        if(confirm('Apakah Anda yakin akan mengahapus data ini?'))
                        {
                                post_response_text(tujuan, param, respon);	
                        }
                        else
