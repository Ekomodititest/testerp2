...
function calculateTotal()
{
        ...
        ttl=parseFloat(bylab)+parseFloat(byadmin)+parseFloat(byobat)+parseFloat(bydr)+parseFloat(byrs)+parseFloat(byfl);
       ...
       //rs		=rs.options[rs.selectedIndex].value;
        ...
        else
        {
                if(confirm(alertsure))
                {
                   ...
                   param+='&byobat='+byobat+'&total='+total+'&bylab='+bylab+'&byfl='+byfl;
...
function editPengobatan(notransaksi,karyawanid,jenisbiaya,lokasitugas,thnplafon,periode,rs,byrs,bydr,bylab,byobat,byadmin,ygberobat,diagnosa,tanggal,total,hariistirahat,klaim,keterangan)
{         
