...
//exit("Error".$jmAwal);
        switch($proses)
        {
                ...
                case'loadData':
					...
					$ql2="select count(*) as jmlhrow from ".$dbname.".sdm_5training  order by `updatetime` desc";// echo $ql2;
					...
					$slvhc="select * from ".$dbname.".sdm_5training   order by `updatetime` desc limit ".$offset.",".$limit." ";
					...
					<td>".tanggalnormal(date('Y-m-d',strtotime($rlvhc['tglmulai'])))."</td>
					<td>".tanggalnormal(date('Y-m-d',strtotime($rlvhc['tglselesai'])))."</td>";
	//atasan==============================                
	...
	//======================================                
					...
					$ql2="select count(*) as jmlhrow from ".$dbname.".sdm_5training where karyawanid!='' ".$cari."  order by `updatetime` desc";// echo $ql2;
					//$ql2="select count(*) as jmlhrow from ".$dbname.".sdm_ijin where karyawanid in (select karyawanid from ".$dbname.".datakaryawan where lokasitugas='".$_SESSION['empl']['lokasitugas']."') order by `tanggal` desc";// echo $ql2;
					$query2=mysql_query($ql2) or die(mysql_error());
					while($jsl=mysql_fetch_object($query2)){
					$jlhbrs= $jsl->jmlhrow;
					...
					//$slvhc="select * from ".$dbname.".sdm_ijin where  karyawanid in (select karyawanid from ".$dbname.".datakaryawan where lokasitugas='".$_SESSION['empl']['lokasitugas']."') order by `tanggal` desc limit ".$offset.",".$limit." ";
					$slvhc="select * from ".$dbname.".sdm_5training  where karyawanid!='' ".$cari."  order by `updatetime` desc limit ".$offset.",".$limit." ";
					$qlvhc=mysql_query($slvhc) or die(mysql_error());
					$user_online=$_SESSION['standard']['userid'];
					while($rlvhc=mysql_fetch_assoc($qlvhc))
					{
