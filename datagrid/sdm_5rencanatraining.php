...
$str="select namakaryawan,karyawanid from ".$dbname.".datakaryawan
     ...
     (tanggalkeluar = '0000-00-00' or tanggalkeluar > '".date("Y-m-d")."') and karyawanid <>".$_SESSION['standard']['userid']. " order by namakaryawan";
$res=mysql_query($str);
...
//kueri atasan penyetuju P.Dinas sesuai setting ==JO 29-11-2016==
$str="select a.karyawanid, b.namakaryawan from ".$dbname.".setup_approval a 
		left join ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid 
		where a.applikasi='PLTA' and a.kodeunit='".substr($_SESSION['empl']['lokasitugas']
,0,4)."' order by b.namakaryawan asc";
$res=mysql_query($str);
$optAts="<option value=''></option>";
while($bar=mysql_fetch_object($res))
{
	$optAts.="<option value='".$bar->karyawanid."'>".$bar->namakaryawan."</option>";
}
//kueri hrd penyetuju P.Dinas sesuai setting ==JO 29-11-2016==
$str="select a.karyawanid, b.namakaryawan from ".$dbname.".setup_approval a 
		left join ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid 
		where a.applikasi='PLTH' and a.kodeunit='".substr($_SESSION['empl']['lokasitugas']
,0,4)."' order by b.namakaryawan asc";
$res=mysql_query($str);
$optHrd="<option value=''></option>";
while($bar=mysql_fetch_object($res))
{
...
echo "<fieldset style='width:700px;'>
	...
	<input type=hidden class=myinputtext id=kodetraining name=kodetraining onkeypress=\"return tanpa_kutip();\" maxlength=30 style=\"width:150px;\" />
    <tr>
    ...
    <tr>
        ...
        <td><input type=text class=myinputtext id=penyelenggara name=penyelenggara onkeypress=\"return tanpa_kutip();\" maxlength=30 style=\"width:150px;\" /></td>
		<!--<select id=penyelenggara name=penyelenggara>".$opthost."</select>-->
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td><select id=persetujuan style=\"width:150px;\">".$optAts."</select></td>
    </tr>
    <tr>
        <td>".$_SESSION['lang']['hrd']."</td>
        <td><select id=hrd style=\"width:150px;\">".$optHrd."</select></td>			 
    </tr>
    </table>	  
    </td>
    <td>  
	...
	<input id=alsimpan type=hidden value='".$_SESSION['lang']['alertinsert1']."'>
	<input id=alselesai type=hidden value='".$_SESSION['lang']['done']."'>
	<input id=cnfdel type=hidden value='".$_SESSION['lang']['confirmhapus']."'>
    </fieldset>
";
//CLOSE_BOX();
...
echo "<fieldset><legend>".$_SESSION['lang']['list']."</legend>
	 ...
	 <input type=hidden id=aldone value='".$_SESSION['lang']['done']."' />
...
while($bar=mysql_fetch_object($res))
{
    ...
    if(($bar->stpersetujuan1==0)&&($bar->sthrd==0))
        echo"<img src=images/application/application_edit.png class=resicon  title='edit' onclick=\"edittraining('".$bar->kode."','".$bar->namatraining."','".$bar->kodejabatan."','".$bar->penyelenggara."','".$bar->hargasatuan."','".tanggalnormal($bar->tglmulai)."','".tanggalnormal($bar->tglselesai)."','".$bar->persetujuan1."','".$bar->persetujuanhrd."','".str_replace("\n", "\\n",$bar->desctraining)."','".str_replace("\n", "\\n",$bar->output)."');\">"; // 
    echo"</td>
    <td>";
    if(($bar->stpersetujuan1==0)&&($bar->sthrd==0))
