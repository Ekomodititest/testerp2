...
function previewDetail(kodeorg,periode,tanggalmulai,tanggalsampai,ev){
        showDetail(kodeorg,periode,tanggalmulai,tanggalsampai,ev);
        param='kodeorg='+kodeorg+'&periode='+periode+'&tanggalmulai='+tanggalmulai+'&tanggalsampai='+tanggalsampai;
        tujuan='log_slave_save_viewtrans.php';
        post_response_text(tujuan, param, respog);
        function respog(){
		  if(con.readyState==4) {
				if (con.status == 200) {
...
function showDetail(kodeorg,periode,tanggalmulai,tanggalsampai,ev){
	title="Detail";
	content="<fieldset><div id=contDetail style='overflow:auto; width:400px; height:300px;'></div></fieldset>";
	width='500';
	height='300';
