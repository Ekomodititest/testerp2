...
function parseDong(tex)
{
	...
	pengajuid	=xmlobject.getElementsByTagName('pengajuid')[0].firstChild.nodeValue;
     	pengajuid=pengajuid.replace("*","");
...
function simpanPJD()
{
	pengajuid	= document.getElementById('pengajuid').value;
	...
	if(document.getElementById('hotel').checked==true)
	   ...
	   hotel=0;
		...
		if (karyawanid == '' || kodeorg == '' || persetujuan == '' || hrd == '' ||  tanggalperjalanan=='') {
			//alert(' Employee, Org.Code, Traveling date, Approval, first destination are obligatory');
			alert('Karyawan, Kode Perusahaan, Tanggal perjalanan, Persetujuan harus diisi');	
		}
		else if ((tujuan1 == '' && tujuan2 == '' && tujuan3 == '' && tujuanlain == '')){
			alert(' Tujuan Perjalanan Dinas harus diisi');
			
		}
		else {
			param ='karyawanid='+karyawanid+'&pengajuid='+pengajuid+'&kodeorg='+kodeorg;
...
function delPJD(nosk,karid)
{
		...
		tujuan='sdm_slave_savePJDinas.php';
		if(confirm('Anda yakin ingin menghapus dokumen '+nosk+ ' ..?'))
		  post_response_text(tujuan, param, respog);	
		function respog(){
			if (con.readyState == 4) {
				if (con.status == 200) {
...
function ganti(keuser,kolom,notransaksi){
		...
		if(confirm('Anda yakinin ingin mengganti persetujuan untuk '+notransaksi+' ..?'))
                    post_response_text(tujuan, param, respog);	
		function respog(){
			if (con.readyState == 4) {
				if (con.status == 200) {
