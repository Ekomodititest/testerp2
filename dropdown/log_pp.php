...
// alert(jdl_ats_0);
 ...
 content_2='<fieldset><legend><?php echo 'Cari';?></legend>Find<input type=text class=myinputtext id=no_cip><button class=mybutton onclick=findCIP()>Find</button></fieldset><div id=container></div>';
...
?>
<fieldset>
<legend><b><?php echo $_SESSION['lang']['list_pp'];?></b></legend>
...
<div style="overflow:scroll; height:420px;">
	 ...
	 <td align='center'><?php echo $_SESSION['lang']['dbuat_oleh']?>
...
<?php
	$strorg 	= "SELECT induk FROM ".$dbname.".organisasi WHERE kodeorganisasi = '".$_SESSION['empl']['lokasitugas']."' LIMIT 0,1";
	$resorg 	= mysql_query($strorg);
	$resa 		= mysql_fetch_assoc($resorg);
	$indukorg 	= $resa["induk"]; 	
	
	$optBagian='';
	if($_SESSION['empl']['tipelokasitugas']== $_SESSION['org']['kodeho']){
		$str="select kodeorganisasi,namaorganisasi from ".$dbname.".organisasi where induk='".$indukorg."'"; //echo $str;
	}
	else{
		$str="select kodeorganisasi,namaorganisasi from ".$dbname.".organisasi where `kodeorganisasi`='".$_SESSION['empl']['lokasitugas']."'"; //echo $str;
	}
	$res=mysql_query($str) or die(mysql_error($conn));
?>
<fieldset>
	<legend><b><?php echo $_SESSION['lang']['prmntaanPembelian']?></b></legend>
	<table cellspacing="1" border="0" id="opl">
	 <tr>
		...
		<tr>
			...
			<button class=mybutton id="dtl_pem" onclick=detailPembelian()><?php echo $_SESSION['lang']['pilih']?></button>			</td>
		</tr>
	</table>
</fieldset>
<fieldset>
<legend><b><?php echo $_SESSION['lang']['detailprmntaanPembelian']?></b></legend><br />
