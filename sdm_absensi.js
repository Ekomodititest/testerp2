...
{
	document.getElementById('syncView').style.display='block';
...
function PilihData(){
	var typesync = document.getElementById('typesync').options[document.getElementById('typesync').selectedIndex].value;
	var periodeysnc = document.getElementById('periodeysnc').options[document.getElementById('periodeysnc').selectedIndex].value;
	
	if(typesync == '') {
		alert('Type Sync Cannot Null');
		return;
	}
	
	if(periodeysnc == '') {
		alert('Periode Cannot Null');
		return;
	}

	param = 'typesync='+typesync+'&periodeysnc='+periodeysnc;
    tujuan ='sdm_slave_absensi_2.php';
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					if(con.responseText == 'Tidak ada data') {
						alert(con.responseText);
					} else {
						document.getElementById('RawDataSync').innerHTML=con.responseText;
					}
				}
			}
			else {
...
function addDetail() {
	...
	if(status_inputan==0)
	{
		if(confirm('Apakah Anda yakin menambahkan detil ?'))
		{
			cek_data();	
		}
	}
...
{
                param='proses=loadNewDataAbsensi';
                param+='&page2='+num;
                tujuan = 'sdm_slave_absensi.php';
                post_response_text(tujuan, param, respog);			
                function respog(){
                        if (con.readyState == 4) {
                                if (con.status == 200) {
                                        ...
                                        if (!isSaveResponse(con.responseText)) {
                                                ...
                                                document.getElementById('containabsensi').innerHTML=con.responseText;
                                        }
                                }
                                else {
                                        busy_off();
...
function delDataLibur(kdorg,tgl)
{
        method = 'delete';
		param='kdorg='+kdorg+'&tgllibur='+tgl+'&method='+method;
...
function saveHariLibur()
{
	...
	if(tgllibur == '') {
		alert('Tanggal harus di isi'); return false;
	}
	method = 'insert';
    param='jnlibur='+jnlibur+'&tgllibur='+tgllibur+'&method='+method;
    tujuan='sdm_slave_absenLibur.php';
